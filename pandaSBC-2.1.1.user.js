...
`);
    function makeAbortable(fn) {
        return function (...args) {
            ...
            const abortP = new Promise((_, rej) => {
                ...
                s.addEventListener('abort', () => rej(new Error('Aborted')), { once: true });
            });
    ...
    function _showHiRatedPopup(title = `本次高分球员（≥98）`) {
        ...
        summary.textContent = `本次共 ${_hiRatedPlayers.length} 名 ≥${highRatingPlayerThreshold} 分，最高 ${maxRating}。快谢谢群主`;
    ...
    async function _doSBC() {
        ...
        const threshold = isTOTW ? MAX_RATING_TOTW : MAX_RATING_NORMAL;
        const hiList = hasHighRating(squad, threshold);
        if (hiList.length > 0) {
            ...
            throw new Error('[doSBC] HighRatedInSquad');
        }
    ...
    async function _tryLoopWithBackoff() {
        const now = Date.now();
    ...
    async function _runVar89NTimes(targets, times = 3, { openAfter = true } = {}) {
        if (abortCtrl?.signal?.aborted) throw new Error('Aborted');
        ...
        if (openAfter && okCount > 0) {
            try {
                await openPacksAfterDoSBC(selectedDoSbcSetId, okCount);
            }
            catch (e) {
        ...
        if (okCount === 0) {
            throw new Error('[runVar89NTimes] 一次都没成功');
        }
        return okCount;
    }
    ...
    async function _autoRound() {
        ...
        if (act === 'try_loop') {
            ...
            try {
                ...
                if (String(e?.message || '').includes('Aborted') || abortCtrl?.signal?.aborted) {
                    throw e;
                }
                await runVar89NTimes(targets, 3, { openAfter: true });
                return true;
            ...
            if (targets.totwId) {
                ...
                try {
                    ...
                    if (!ok && targets.var89Id) {
                        ...
                        await openPacksAfterDoSBC(selectedDoSbcSetId, 1).catch(() => { });
                    }
                    return true;
                } catch (e) {
                    if (String(e?.message || '').includes('Aborted') || abortCtrl?.signal?.aborted) {
    ...
    const tryLoopWithBackoff = makeAbortable(_tryLoopWithBackoff);
    const runVar89NTimes = makeAbortable(_runVar89NTimes);
    function startLoop() {
        startAsyncTask({
            button: btnLoop,
            taskName: 'loop',
