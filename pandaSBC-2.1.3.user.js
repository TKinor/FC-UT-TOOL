  ...
  const PandaSBC = (() => {
    ...
    const sbc = {
      ...
      async addPlayer() {
        ...
        if (!(await dom.waitForElement('.ut-image-button-control.filter-btn.custom-player-add', 5000)))
          return;
    ...
    `;
      ...
      async loopOnce() {
        ...
        try {
          ...
          if (unassignedCount !== 10) {
            ...
            if (retry < 1) {
              return await sbc.loopOnce(retry + 1);
            } else {
              return false;
            }
          }
      ...
      async runSbcNTimesPreferOpen(setId, times = 3, { openAfter = true } = {}) {
        if (!setId) return 0;
      ...
      async runTotwNTimes(targets, times = 1, { openAfter = true } = {}) {
        if (!targets?.totwId) return 0;
        return sbc.runSbcNTimesPreferOpen(targets.totwId, times, { openAfter });
      },
      async autoRound() {
        if (state.abortCtrl?.signal?.aborted) throw new AbortedError();
        ...
        const FAILS_BEFORE_FB = config?.PRO?.TRY_LOOP_FAILS_BEFORE_FALLBACK ?? 2;
        const AUX_MAX_CONSEC = config?.PRO?.AUX_MAX_CONSEC ?? 2;

        state._tryLoopFailStrike2 ??= 0;
        state._tryLoopFbUsedForStrike ??= false;

        state._auxActGuard ??= { lastAct: null, consec: 0 };
        const stepAuxGuard = (thisAct) => {
          if (thisAct === 'do_totw' || thisAct === 'do_var89') {
            const g = state._auxActGuard;
            if (g.lastAct === thisAct) g.consec += 1;
            else { g.lastAct = thisAct; g.consec = 1; }
            return g.consec <= AUX_MAX_CONSEC;
          } else {
        ...
        const resetTryLoopStrike = () => {
          state._tryLoopFailStrike2 = 0;
          state._tryLoopFbUsedForStrike = false;
        };

        const fallback89 = async () => {
          if (!targets?.var89Id) return false;
          try {
            const ok89 = await sbc.runVar89NTimes(targets, 3, { openAfter: true });
            if (!ok89) { log.w('89x3 failed stopping auto.'); return false; }
            return true;
          } catch (e) {
            if (isAbort?.(e)) throw e;
            log.w('[auto] fallback89 error:', e);
            return false;
          }
        };
        ...
        if (act === 'try_loop') {
          ...
          if (!loopId) { log.i('[auto] 没有可用的SBC'); return true; }
          ...
          try {
            ...
            state._tryLoopFailStrike2++;

            if (!state._tryLoopFbUsedForStrike && state._tryLoopFailStrike2 >= FAILS_BEFORE_FB) {
              state._loopFailStrike = 0;
              state._lastLoopFailAt = 0;
              const okFb = await fallback89();
              state._tryLoopFbUsedForStrike = true;
              return true;
            }
        ...
        if (act === 'do_totw') {
          if (!stepAuxGuard('do_totw')) {
            log.w('[auto] do_totw 达到连续上限，跳过（回到主线）');
            return false;
          }
          ...
          try {
            const ok = await sbc.runTotwNTimes(targets, 1, { openAfter: true });
            return true;
          } catch (e) {
            if (isAbort?.(e)) throw e;
        ...
        if (act === 'do_var89' && targets?.var89Id) {
          if (!stepAuxGuard('do_var89')) {
            log.w('[auto] do_var89 达到连续上限，跳过（回到主线）');
            return false;
          }

          try {
            await sbc.runVar89NTimes(targets, 3, { openAfter: true });
            return true;
          } catch (e) {
            if (isAbort?.(e)) throw e;
    ...
    const tasks = {
      ...
      _stopPromise: null,
      async stopAsync({ timeout = 8000, minHold = 300 } = {}) {
